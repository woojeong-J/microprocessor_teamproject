...
void PORT_init_Motor(void)
{
	...
	PORTC_PCR17 |= (0b1000<<IRQC_BITS); // Interrupt on falling edg
...
void PORTC_IRQHandler(void)
{
	...
	Accel_Flag = 1;
	Brake_Flag = 0;
	PORTC_PCR17 |= (1<<ISF_BIT); // 인터럽트 플래그 클리어
}
...
void PORTA_IRQHandler(void)
{
	...
	Brake_Flag = 1;
	Accel_Flag = 0;
	PORTA_PCR12 |= (1<<ISF_BIT); // 인터럽트 플래그 클리어
}
...
void DRV_Control()
{
	...
	{
		current_speed = current_speed + (limited_speed/(speed_time * 100)); // 가속도(s단위) 에 따라 속도 증가
	...
	FTM2_C0V = (current_speed * FTM2_MOD) / max_speed;
}
...
void DRV_Brake_Control()
{
	...
	if(current_speed > 0)
	{
		current_speed = current_speed - (max_speed/(5 * 100)); // 5초만에 정지
		if(current_speed < 0)
			current_speed = 0;
	}
	FTM2_C0V = (current_speed * FTM2_MOD) / max_speed; // PWM duty 변경
}
...
void DRV_Coasting_Control()
{
	...
	if(current_speed > 0)
	{
		current_speed = current_speed - (max_speed/(20 * 100)); // 20초만에 정지
		if(current_speed < 0)
			current_speed = 0;
	}
	FTM2_C0V = (current_speed * FTM2_MOD) / max_speed;
